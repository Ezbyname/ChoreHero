...
test('maps a plain open task to a task-kind activity with a complete action', () => {
...
test('completed tasks expose no actions', () => {
  const activity = TaskAdapter.toFamilyActivity(makeTask({ status: 'completed' }));
